...
module Assert =
        ...
        let equalDeepHelper<'a> (selfReference:MethodInfo) (expected:'a) (actual:'a) (breadcrumb:string list) =
            ...
            let (|ListToList|_|) (candidate : obj) = Some ((candidate :?> System.Collections.IEnumerable) |> Seq.cast<obj> |> Seq.toList)
            let (|SeqToSeq|_|) (candidate : obj) = Some ((candidate :?> System.Collections.IEnumerable) |> Seq.cast<obj>)
            let (|ArrayToArray|_|) (candidate : obj) = Some ((candidate :?> System.Collections.IEnumerable) |> Seq.cast<obj> |> Seq.toArray)
            ...
            let formatError (message:string) (breadcrumbValue:string option) : string =
                let breadcrumbFolder (state:string) (breadcrumbItem:string) : string =
                    if state.Length = 0 then
                        ...
                        sprintf "%s.%s" breadcrumbItem state
            ...
            let compare (expected:'b) (actual:'b) (message:string) (breadcrumbValue:string option) =
                if expected <> actual then
                    raise (new EqualDeepException (formatError message breadcrumbValue))
            ...
            let recurse (expected:'b) (actual:'b) (t:Type) (breadcrumbValue:string) =
                ...
                if t.Name = "String" then
                    compare expected actual (sprintf "String values for %A do not match. Expected '%A' but actual was '%A'" t.Name expected actual) (Some breadcrumbValue)
                else
            ...
            if t |> FSharpType.IsRecord then
                ...
                if eUnionFields.Length = 0 || aUnionFields.Length = 0  then
                    compare eUnionCaseInfo.Name aUnionCaseInfo.Name (sprintf "At least one union only has a label. Expected type was %A but the actual was %A" eUnionCaseInfo.Name aUnionCaseInfo.Name) (Some eUnionCaseInfo.Name)
                else
                    match (eUnionFields.[0], aUnionFields.[0]) with
                    | (null, null) -> ()
                    | (null, a) -> fail (sprintf "Field %A is expected to be None or null, but it is %A" eUnionCaseInfo.Name a) (Some eUnionCaseInfo.Name)
                    | (e, null) -> fail (sprintf "Field %A is expected to be %A, but it is None or null" eUnionCaseInfo.Name e) (Some eUnionCaseInfo.Name)
                    | (e, a) ->
                        ...
                        if e.GetType() |> FSharpType.IsRecord then
                            assertRecord (e.GetType()) e a
                        else compare (sprintf "%A" expected) (sprintf "%A" actual) (sprintf "Value is expected to be '%A' but is '%A'" expected actual) (Some eUnionCaseInfo.Name)
        ...
        let equalDeep<'a> (expected:'a) (actual:'a) =
            ...
            with _ ->
                let t = typeof<'a>
                if FSharpType.IsRecord t || FSharpType.IsUnion t || (t.IsGenericType && t.GetGenericTypeDefinition() = typedefof<list<_>>)
                        || (t.IsGenericType && t.GetGenericTypeDefinition() = typedefof<seq<_>>) || t.IsArray || FSharpType.IsTuple t then
                    let moduleInfo =
        ...
        let success choice =
            match choice with
            | Choice1Of2 _ -> ()
            | Choice2Of2 _ -> raise (new XunitException("Expected Success but got Failure"))

        let failure choice =
            match choice with
            | Choice1Of2 _ -> raise (new XunitException("Expected Failure but got Success"))
            | Choice2Of2 _ -> ()
